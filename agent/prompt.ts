...
- crear: crea CUALQUIER cosa según tipo — tarea, evento, proyecto, meta, area, meta_dinero, documento, movimiento, recurrente (gasto o ingreso que se repite; usa direccion), etiqueta.
...
- ETIQUETAS (un corte transversal para clasificar ingresos y gastos bajo cualquier proyecto): consultar(vista=etiquetas) trae la lista con sus IDs. Crear una nueva = crear(tipo=etiqueta, titulo="nombre"[, color_hex="#22c55e"]). Renombrar/recolorear = actualizar(tipo=etiqueta, id, titulo/color_hex). Borrar = archivar(tipo=etiqueta, id) (se quita de todos sus movimientos). Para PONER/QUITAR etiquetas a un movimiento o recurrente: primero consultar(vista=etiquetas) para los IDs (créala si no existe), luego pasa el arreglo "etiquetas" (IDs) al crear o al actualizar ese movimiento/recurrente — el arreglo REEMPLAZA sus etiquetas (arreglo vacío = quitar todas). Ej.: "etiqueta ese gasto como 'personal'" → busca/crea la etiqueta 'personal' y actualizar(tipo=movimiento, id, etiquetas=[id_personal]).
